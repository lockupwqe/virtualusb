...
public class SessionThread extends Thread {
	...
	public void run() {
		...
		String localLanguage = Globals.getContext().getResources().getConfiguration().locale.getLanguage();
		String localCounty = Globals.getContext().getResources().getConfiguration().locale.getCountry();
		if (Locale.CHINA.getCountry().equals(localCounty)) {
			setEncoding("GB2312");
		} */
		...
		try {
			BufferedReader in = new BufferedReader(new InputStreamReader(cmdSocket
					.getInputStream(), getEncoding()), 8192); // use 8k buffer,8192  16k modify
			while (true) {
				String line;
